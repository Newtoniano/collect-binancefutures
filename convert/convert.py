        ...
        while True:
            ...
            if data is not None:
                ...
                if evt == "trade":
                    ...
                    # event_time = data['E']
                    transaction_time = data["T"] if data.get("T") else data["E"]
                    bids = data["b"]
                    asks = data["a"]
                    exch_timestamp = int(transaction_time) * 1000
                    if correct_exch_timestamp and exch_timestamp < prev_exch_timestamp:
                ...
                # this is for the spot book ticker stream response that doesn't have an event key
                if not evt and "@bookTicker" in message.get("stream") and args.full:
                    # event_time = data['E']
                    # transaction_time = data["T"]
                    bid_price = data["b"]
                    bid_qty = data["B"]
